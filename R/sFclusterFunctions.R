## Functions taken from package snowfall (http://cran.r-project.org/web/packages/snowfall/index.html)
...
##    return( invisible( TRUE ) )
  ...
  for( name in names ) {
    ## Also examine namespace (from cn.farms package?). Only needed for internal
    ## functions.
...
##      }
  ...
  testSource <- function() {
    ...
    if( is.null( res ) )
      return( c( FALSE, "Cannot locate package cn.farms." ) )
...
#    var2 <<- 101
    ...
    ## Setting var in namespace ("cn.farms").
    ...
    iTest <- function() {
      ...
      res <- sfExport( "var1", "var2", ".sfTestVar5",
                       list=list( "var3", "var4" ),
                       local=TRUE, namespace="cn.farms", stopOnError=FALSE )
