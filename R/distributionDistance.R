...
distributionDistance <- function(
	...
	if(!is.matrix(intensityData)){
		...
		if(class(intensityData)=="AffyBatch"){
			intensityData <- pm(intensityData) 
		}
	    
		if(class(intensityData)=="ExpressionSet"){
			intensityData <- exprs(intensityData) 
		}
	    intensityData <- as.matrix(intensityData)
        
	}
	
	nRows <- nrow(intensityData)
	nCols <- ncol(intensityData)
	...
	if (useSubset){ 
		...
		set.seed(123)    
		### get data subset
		intensityData <- intensityData[sample.int(nRows, 
                        floor(nRows * subsetFraction)),]
	}
		...
		### compute quantile distribution as reference 
      	referenceDistribution <- apply(apply(intensityData, 2, sort), 1, median)
       	...
       	for(i in 1:nCols){
			...
			res[i] <- calcDistance(x=intensityData[,i], 
                    y=referenceDistribution, method=method)
		}
		
		attributes(res) <- list(Size = nCols, Labels = colnames(intensityData), 
		        methods=method, class = "vector")
	} else {
       	res <- rep(NA, nCols * (nCols - 1) / 2)
   		count <- 1
   		### compute pairwise distances
		for(i in 1:(nCols - 1)){
   			for(j in (i+1):nCols){
      			res[count] <- calcDistance(x=intensityData[,i], 
                        y=intensityData[,j], method=method)
        		count <- count+1
           	}
        }
	    
		attributes(res) <- list(Size = nCols, Labels = colnames(intensityData), 
		        methods=method, class = "dist")
