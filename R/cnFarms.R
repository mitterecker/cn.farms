...
cn.farms <- function (filenames, cores = 1, runtype = "bm") {
  ...
  normData <- normalizeCels(filenames, method=normMethod, cores = cores, alleles = TRUE, runtype = runtype)
  ...
  if (separateAnalysis) {
    mlDataSnp <- mlSummarization(slData, windowMethod, windowParam, 
        summaryMethod, summaryParam, callParam = callParam, saveFile = "mlSnp")  
    
    mlDataCn <- mlSummarization(npData, windowMethod, windowParam, 
        summaryMethod, summaryParam, callParam = callParam, saveFile = "mlCn")  
  }
